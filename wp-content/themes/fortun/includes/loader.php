...
add_action( 'init', 'fortun_bunch_theme_init');


if( !function_exists( 'fortun_set' ) ) {
	function fortun_set( $var, $key, $def = '' )
	{
		if( !$var ) return false;
...
if( !function_exists('fortun_printr' ) ) {
	function fortun_printr($data)
	{
		echo '<pre>'; print_r($data);exit;
	}
}

if( !function_exists('fortun__font_awesome' ) ) {
	function fortun_font_awesome( $index )
	{
		$array = array_values($GLOBALS['_font_awesome']);
		if( $font = fortun_set($array, $index )) return $font;
		else return false;
	}
}

if( !function_exists('fortun_load_class' ) ) {
	function fortun_load_class($class, $directory = 'libraries', $global = true, $prefix = 'Bunch_')
	{
...
fortun_load_class( 'enqueue', 'helpers', false );
fortun_load_class( 'bootstrap_walker', 'helpers', false );


if( fortun_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...
function fortun_bunch_theme_init()
{
	 ...
	 */
	// metaboxes
	$tmpl_mb1  = include FORTUN_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
	// * Create instances of Metaboxes
	foreach( $tmpl_mb1 as $tmb )  new VP_Metabox($tmb);
	
	$tmpl_mb1  = include FORTUN_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
	...
	$tmpl_sg1  = FORTUN_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
	$tmpl_sg2  = FORTUN_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';
	...
	if( is_admin() ) {
		...
		if( fortun_set( $_GET, 'page' ) == 'fortun'.'_option' ) 
		{
			if( fortun_set( $_GET, 'bunch_dummydata_export' ) ){
	...
	if( function_exists( 'fortun_vc_map' )) 
	include_once get_template_directory() .  '/vc_map.php' ;
	
	// options
	$tmpl_opt  = FORTUN_ROOT.'includes/vafpress/admin/option/option.php';
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'fortun'.'_theme_options',                      // options key in db, required
		'page_slug'             => 'fortun'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'fortun' ), // page title
		'menu_label'            => esc_html__( 'Theme Options', 'fortun' ), // menu label
	));
